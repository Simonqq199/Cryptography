...
void runBenchmark(CryptoPP::StreamTransformation& cipher, CryptoPP::AutoSeededRandomPool& prng)
{
...
int main(int argc, char* argv[])
{
    ...
    CryptoPP::CTR_Mode<CryptoPP::DES>::Encryption desCipher;
    desCipher.SetKeyWithIV(desKey, desKey.size(), desKey);

    runBenchmark(desCipher, prng);

    // Benchmark AES encryption
    CryptoPP::SecByteBlock aesKey(CryptoPP::AES::DEFAULT_KEYLENGTH);
    prng.GenerateBlock(aesKey, aesKey.size());

    CryptoPP::CTR_Mode<CryptoPP::AES>::Encryption aesCipher;
    aesCipher.SetKeyWithIV(aesKey, aesKey.size(), aesKey);

    runBenchmark(aesCipher, prng);
